...
const login = [check('email', 'The email must be valid').trim().notEmpty().normalizeEmail().isEmail().toLowerCase(), check('password', 'Password is required').trim().notEmpty(), validateErrors];

const forgetPassword = [check('email', 'The email must be valid').trim().notEmpty().normalizeEmail().isEmail().toLowerCase(), validateErrors];

const newPassword = [check('password', 'The password must be at least 6 characters').isLength({ min: 6 }), validateErrors];
